...
class AdminMenuModel extends Model {
	...
	const ADMIN_MENU = "db_ysb_admin.ts_menu";
	...
	protected $_validate = array(
			...
			array('app,model,action', 'checkAction', '同样的记录已经存在！', 1, 'callback', 1   ),
			array('id,app,model,action', 'checkActionUpdate', '同样的记录已经存在！', 1, 'callback', 2 ),
	...
	public function get_tree($myid, $parent = "", $Level = 1) {
		...
		//选中的菜单
		$men = $this->where(array('model'=>CONTROLLER_NAME,'action'=>ACTION_NAME))->find();
		$selectArray[] = $men['id'];
		if($men['parentid']){
			$selectArray[] = $men['parentid'];
			$men = $this->where(array('id'=>$men['parentid']))->find();
			$men['parentid'] ? $selectArray[] = $men['parentid'] : false;
		}	
		$selectArray = $selectArray ? $selectArray : array(-1);
		...
		if (is_array($data)) {
			foreach ($data as $a) {
				...
				if(in_array($a['id'],$selectArray)){
					$ret[$id . $name]['isSelect']  = 1; //是否选中
				}
				$child = $this->get_tree($a['id'], $id, $Level);
				//由于后台管理界面只支持三层，超出的不层级的不显示
				if ($child && $Level <= 3) {
					$ret[$id . $name]['items'] = $child;
					$ret[$id . $name]['isParent']  = 1; //是否写url
				}
